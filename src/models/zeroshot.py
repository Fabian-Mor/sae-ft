...
def get_zeroshot_classifier(args, clip_model):
    ...
    if _OPENCLIP_ENABLED:
        clean_name = args.model
        if ':' in clean_name:
            clean_name = clean_name.split(':')[1]
        tokenizer = _openclip.get_tokenizer(clean_name)
    else:
        tokenizer = None

    print('Getting zeroshot weights, with context length {}'.format(context_length))
    with torch.no_grad():
        zeroshot_weights = []
        for classname in tqdm(dataset.classnames):
            ...
            texts = clip.tokenize(texts, context_length=context_length, tokenizer=tokenizer).to(device) # tokenize
