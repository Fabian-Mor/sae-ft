...
class IWildCamNonEmpty(IWildCam):
    def __init__(self, *args, **kwargs):
        kwargs['subset'] = 'train'
        super().__init__(*args, **kwargs, remove_non_empty=True)
...
class IWildCamUnlabeled(IWildCam):
    def __init__(self, preprocess, location=os.path.expanduser('~/data'), batch_size=128, num_workers=16):
        self.dataset = wilds.get_dataset(dataset='iwildcam', root_dir=location, unlabeled=True, download=False)

        self.unlabeled_subset = self.dataset.get_subset('extra_unlabeled', transform=None)  # Add your transform here

        # 3. Create loader
        self.unlabeled_loader = get_train_loader(
            "standard",
        ...
        super().__init__(
            preprocess,  # Add your transform here
            location=location,
            remove_non_empty=False,
            batch_size=batch_size,
            num_workers=num_workers
