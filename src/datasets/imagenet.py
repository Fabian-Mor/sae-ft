...
class ImageNet:
    def __init__(self,
                 preprocess,
                 location='/weka/datasets/ImageNet2012',
                 batch_size=32,
                 num_workers=4,
                 classnames='openai'):
    ...
    def get_test_path(self):
        test_path = os.path.join(self.location, 'val_in_folder')
...
def project_logits(logits, class_sublist_mask, device):
    ...
    mask_tensor = torch.tensor(class_sublist_mask, dtype=torch.bool, device=logits.device)

    if logits.size(1) > mask_tensor.sum():
        return logits[:, mask_tensor].to(device)
    else:
